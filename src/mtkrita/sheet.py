...
def split_grid(image: Image.Image, spec: GridSpec | None = None) -> list[ExtractedFrame]:
    ...
    usable_width = image.width - (2 * resolved_spec.margin_x) - ((resolved_spec.columns - 1) * resolved_spec.gap_x)
    usable_height = image.height - (2 * resolved_spec.margin_y) - ((resolved_spec.rows - 1) * resolved_spec.gap_y)
    ...
    for row in range(resolved_spec.rows):
        for column in range(resolved_spec.columns):
            ...
            frames.append(ExtractedFrame(index=(row * resolved_spec.columns) + column + 1, row=row, column=column, box=box, image=image.crop(box)))
    return frames
