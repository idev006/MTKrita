...
- each frame may contain:
  - character artwork
  - Thai caption text
  - white sticker outline around character/text/elements
  - props and decorative elements
  - frame sequence badge/number near a corner
- foreground may contain dark/black pixels (hair, text, shadows), therefore global black color-key removal is unsafe

## Mandatory Processing Behavior

### 1. Sheet detection and split
The system shall detect or infer the 2×5 frame layout without assuming a fixed total sheet size. 1536×613 is a supported observed size, not a hard-coded requirement.

### 2. Border removal
Border removal shall occur per-frame after split.

The border detector shall support:
- varying colors
- varying thicknesses
- anti-aliased edges
- rounded corners
- slight resizing/compression effects

The system shall not remove pixels solely because they match the border color.
...
### 4. Dark foreground safety
Black or near-black artwork must be protected when it is not topologically connected to the background region.

Examples include:
- black/dark hair
- dark Thai text strokes
- shadows and accessories
...
The regression suite shall include cases equivalent to:

- RGB opaque sheet, black background, green rounded frame borders
- 2×5 grid
- dark hair touching/near white outline
- black/dark text inside white outline
- decorative elements close to frame boundary
- frame numbering badge near corner
- border of alternate color
- border of alternate width
- resized sheet whose cell geometry is not perfectly divisible
...
## Design Rule
A successful result is not merely 'black background removed'. A successful result must preserve all intended sticker artwork, including dark content and anti-aliased edges, while producing a clean transparent background suitable for downstream LINE export.
