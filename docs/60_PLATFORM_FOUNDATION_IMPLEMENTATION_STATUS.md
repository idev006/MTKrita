...
## Status
SSOT — Platform Foundation Implementation Track v1.6
...
- `MainBoard` composition root containing PathManager, ResourceBroker, JobStore, EventBus, TaskLeaseRegistry, JobLifecycleController and StartupReconciler
- lifecycle/recovery services share the same authoritative JobStore instance
...
- durable equivalent of assignment/lease/attempt state stored in JobStore v2

The in-memory `TaskLeaseRegistry` remains a fast runtime guard, while durable JobStore state is authoritative across restart/recovery boundaries.

#### Pause / stop / resume lifecycle
- SSOT state model extended with `PAUSING`, `PAUSED`, `STOPPING`, `STOPPED` and `INTERRUPTED`
- `JobLifecycleController` owns lifecycle transition policy
- pause is two-phase: request pause, then finalize only after no authoritative RUNNING tasks remain
- stop can supersede PROCESSING/PAUSING/PAUSED and finalizes only at a safe boundary
- resume is allowed from PAUSED, STOPPED or INTERRUPTED only
- invalid lifecycle transitions are rejected before durable mutation
- lifecycle behavior is covered by headless automated tests

#### Startup reconciliation
- `StartupReconciler` scans durable jobs left in PROCESSING/PAUSING/STOPPING by a previous process
- orphaned RUNNING tasks become INTERRUPTED using durable worker/attempt CAS checks
- affected jobs become INTERRUPTED only after task reconciliation
- already durable PAUSED/STOPPED/terminal jobs are not rewritten by startup recovery
- recovery is idempotent: a second pass after successful reconciliation is a no-op
- recovery events are recorded in the durable event journal

### Architectural rule
Workers produce candidate artifacts only in private scratch. Shared/final artifacts are committed through MainBoard-owned ResourceBroker services using PathManager-resolved references. Authoritative job/task state is persisted through a single-logical-writer JobStore; workers never write JobStore directly. Cross-component communication uses the versioned message envelope. Worker results are accepted only when lease/attempt identity is current in both runtime guard and durable state. Lifecycle/recovery services own policy; UI submits commands and does not mutate durable state directly.

### CI status
- PR #10 Ruff + pytest passed after PathManager worker-file regression correction.
- JobStore v2 migration/task-persistence changes passed Ruff + pytest.
- lifecycle/recovery/MainBoard-composition head requires a fresh green CI result before this revision is considered verified.

### Known reliability gap
Startup reconciliation currently performs per-task durable interruption followed by job interruption as separate transactions. This is recoverable and idempotent, but not yet a single cross-record recovery transaction. A future JobStore-level reconciliation transaction should tighten crash consistency before production release.

### Next work packages
1. coordinated ResourceBroker ↔ JobStore artifact/state commit protocol with crash reconciliation
2. structured event/log sink and diagnostic bundle baseline
3. bounded scheduler/backpressure foundation
4. process WorkerManager lifecycle/heartbeat integration
5. tighten startup reconciliation into a store-owned atomic recovery transaction where practical
